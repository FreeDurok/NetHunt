...
USE_DOCKER_ZEEK = True  # Set to False to force local Zeek installation
ZEEK_PATH: Optional[str] = "/opt/zeek/bin/zeek"  # Path to native Zeek installation
...
def analyze(pcap, outdir, chunk=None):
    ...
    if use_zeek_docker:
        if not check_zeek_docker_image():
            print("[INFO] Pulling Zeek Docker image...")
            try:
                subprocess.run(["docker", "pull", ZEEK_DOCKER_IMAGE], check=True, capture_output=True)
            except:
                print("[WARN] Failed to pull Docker image, trying native Zeek...")
    ...
    print("Tool Status:")
    if use_zeek_docker:
        print(f"  ✓ Zeek (Docker): {ZEEK_DOCKER_IMAGE}")
